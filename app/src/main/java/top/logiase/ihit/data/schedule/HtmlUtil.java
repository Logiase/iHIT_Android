...
public class HtmlUtil {
     ...
     * @return
     */
    public List<Subject> getzkb(String xnxq, String usrId){
                    ...
                    for(int k = 0; k < strings.length;){
                        ...
                        try{
                            ...
                            if(strings[k].contains("机械设计A")){
                                ...
                                Subject mySubject = getMyExam(i, j, strings[k], strings[k + 1]);
                                k += 2;
                                if(mySubject != null){
//                                    mySubject.save();
...
//                                        subject1.save();
                                ...
                                Subject mySubject = getMySubject(i, j, strings[k], strings[k + 1], strings[k+2]);
                                k += 3;
                                if(mySubject != null){
//                                    mySubject.save();
                                    mySubjects.add(mySubject);
                                ...
                                Subject mySubject = getMySubject(i, j, strings[k], strings[k + 1]);
                                k += 2;
                                if(mySubject != null){
//                                    mySubject.save();
                                    mySubjects.add(mySubject);
     ...
     * @return
     */
    private Subject getMyExam(int i, int j, String course, String info){
        ...
        // 开始时间 由id决定
        subject.setStart(2*(i-1) + 1);
        // 连上两节课
        subject.setStep(2);

        subject.setInfo(info.split("周")[0] + "周");

        // 使用[]来分割为3部分，分别为 教师姓名 上课周数 上课地点
        String [] infos = info.split("[\\[\\]]");

        subject.setRoom("无");
        subject.setWeekList(getWeeks(infos[1], false, false));
     ...
     * @return
     */
    private Subject getMySubject(int i, int j, String course, String info){
        Subject subject = new Subject();
        // 周几，由第几列决定
        subject.setDay(j-1);
        // 课程名  split分割的偶数位为课程名
...
//        mySubject.setInfo(info.split("周")[0] + "周");
        ...
        subject.setInfo(info.substring(0, index));
        ...
        subject.setTeacher(infos[0]);
        subject.setRoom(infos[2].replace("周",""));
        subject.setWeekList(getWeeks(infos[1], isEven, isOdd));
     ...
     * @return
     */
    private Subject getMySubject(int i, int j, String course, String info, String room){
        Subject subject = new Subject();
        // 周几，由第几列决定
        subject.setDay(j-1);
        // 课程名  split分割的偶数位为课程名
     ...
     * @return
     */
    public List<Subject> getMySubject1(int i, int j, String course, String info){
        ...
        for(String each : infos){
            ...
            // 借助解析一个老师的函数来进行
            Subject subject = getMySubject(i, j, course, s);
     ...
     * @return
     */
    public List<Subject> getMySubject1(int i, int j, String course, String info, String room){
